...
class ModelLoader:
    ...
    def __init__(self, model_path=None):
        ...
        if model_path and os.path.exists(model_path):
            ...
            possible_paths = [
                "modeli.p",
                os.path.join(os.path.dirname(__file__), "modeli.p"),
                os.path.join(os.path.expanduser("~"), "Desktop", "American-Sign-language-Detection-System", "modeli.p")
            ]
...
class EnhancedASLApp(QMainWindow):
    """Enhanced ASL Recognition Application with Gemini Integration"""
    ...
    def __init__(self):
        ...
        self.labels_dict.update({26 + i: str(i) for i in range(10)})
        self.labels_dict[36] = ' '
        self.labels_dict[37] = '.'
    ...
    def create_help_tab(self):
        ...
        <p>This application recognizes American Sign Language (ASL) gestures through your webcam and converts them to text. It uses Gemini AI to enhance text for natural speech and word suggestions.</p>
        ...
        <ul>
            <li><b>🔊 Speak</b>: Speaks the current sentence aloud, enhanced by Gemini AI</li>
        ...
        <h3>About</h3>
        <p>Enhanced ASL Recognition System v2.1 - Gemini Edition</p>
        <p>This application uses MediaPipe for hand tracking, machine learning for ASL recognition, and Gemini AI for text enhancement.</p>
        """)
    ...
    def update_suggestions(self):
        """Update word suggestions with Gemini enhancement if enabled"""
        ...
        try:
            ...
            if hasattr(self, 'use_gemini') and self.use_gemini and self.word_buffer:
                try:
                    prompt = (
                        f"Given the current sentence: '{self.sentence}' and the partial word: '{self.word_buffer}', "
                        f"suggest up to 5 relevant words that fit the context. Return only the words, one per line."
                    )
                    response = self.gemini_model.generate_content(prompt)
                    gemini_suggestions = response.text.strip().split('\n')[:5]
                    suggestion_terms = list(dict.fromkeys(suggestion_terms + gemini_suggestions))[:5]
                except Exception as e:
                    print(f"Gemini suggestion error: {e}")
                    suggestion_terms = suggestion_terms[:5]
            
            for i in range(5):
                if i < len(suggestion_terms):
                    self.suggest_buttons[i].setText(suggestion_terms[i])
    ...
    def speak(self):
        """Speak the current sentence with Gemini-enhanced text"""
        ...
        try:
            self.status_bar.showMessage("Processing and speaking...")
            ...
            if hasattr(self, 'use_gemini') and self.use_gemini:
                try:
                    prompt = (
                        f"Refine the following text for natural speech, correcting grammar and making it more coherent: "
                        f"'{text_to_speak}'. Ensure the output is concise and suitable for text-to-speech."
                    )
                    response = self.gemini_model.generate_content(prompt)
                    enhanced_text = response.text.strip()
                    if enhanced_text:
                        text_to_speak = enhanced_text
                        self.status_bar.showMessage(f"Enhanced text: {text_to_speak}")
                    else:
                        print("⚠️ Gemini returned empty response, using original text")
                except Exception as e:
                    print(f"Gemini API error: {e}")
                    self.status_bar.showMessage("Gemini processing failed, using original text")
...
def main():
    ...
    app.setApplicationVersion("2.1 - Gemini Edition")
    
    window = EnhancedASLApp()
    window.show()
    
    print("🚀 Enhanced ASL Recognition System Started!")
    print("✅ GEMINI EDITION - Enhanced with Gemini AI!")
    print("✅ All buttons are fully functional:")
    print("   🔊 Speak - Uses Gemini for text enhancement before TTS")
