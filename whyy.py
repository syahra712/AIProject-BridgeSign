    ...
    def __init__(self, model_path=None):
        ...
        if model_path and os.path.exists(model_path):
            try:
                ...
                # Load label encoder
                label_encoder_path = os.path.join(os.path.dirname(model_path), 'label_encoder.pickle')
                if os.path.exists(label_encoder_path):
                    self.label_encoder = pickle.load(open(label_encoder_path, 'rb'))
                    if len(self.label_encoder.classes_) != self.expected_classes:
                        print(f"⚠️ Warning: Label encoder has {len(self.label_encoder.classes_)} classes, but {self.expected_classes} are expected.")
            ...
            possible_paths = [
                "model.p",  # Check for model.p first
                "modelbest.p",
                os.path.join(os.path.dirname(__file__), "model.p"),
                os.path.join(os.path.dirname(__file__), "modelbest.p"),
                os.path.join(os.path.expanduser("~"), "Desktop", "American-Sign-language-Detection-System", "model.p"),
            ...
            for path in possible_paths:
                if os.path.exists(path):
                    try:
                        ...
                        self.model = model_dict['model']
                        label_encoder_path = os.path.join(os.path.dirname(path), 'label_encoder.pickle')
                        if os.path.exists(label_encoder_path):
                            self.label_encoder = pickle.load(open(label_encoder_path, 'rb'))
                            if len(self.label_encoder.classes_) != self.expected_classes:
                                print(f"⚠️ Warning: Label encoder has {len(self.label_encoder.classes_)} classes, but {self.expected_classes} are expected.")
...
def __init__(self):
    ...
    # Setup labels dictionary using label encoder if available
    if self.model_loader.label_encoder:
        self.labels_dict = {i: str(label) for i, label in enumerate(self.model_loader.label_encoder.classes_)}
    else:
        # Fallback labels dictionary
